...
import type { PatAuthRequest } from "../../packages/shared/src/protocol.js";
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const result = await createBffFromEnv(request).authWithPat(parseJsonBody<PatAuthRequest>(request));
