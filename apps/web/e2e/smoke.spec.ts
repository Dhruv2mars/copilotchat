...
test("pair, connect, chat", async ({ page, request }) => {
  await request.post("http://127.0.0.1:8787/auth/logout");
  ...
  await expect(page.getByRole("heading", { name: "Connect Copilot" })).toBeVisible();

  await page.getByRole("button", { name: "Connect Copilot" }).click();
  await expect(page.getByRole("heading", { name: "Local Copilot" })).toBeVisible();
  ...
  await expect(page.getByText("Local bridge online. Ship it")).toBeVisible();
